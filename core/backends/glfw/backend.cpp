...
#define OPENGL_VERSION_COUNT (sizeof(OPENGL_VERSIONS_GLSL) / sizeof(char*))
    ...
    int init(std::string resDir) {
        ...
        // Load app icon
        if (!std::filesystem::is_regular_file(resDir + "/icons/monobs512.png")) {
            flog::error("Icon file '{0}' doesn't exist!", resDir + "/icons/monobs512.png");
            return 1;
        }

        GLFWimage icons[10];
        icons[0].pixels = stbi_load(((std::string)(resDir + "/icons/monobs512.png")).c_str(), &icons[0].width, &icons[0].height, 0, 4);
